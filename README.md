...
全局设置页可按标准 OpenCC 规则显示简体或繁体字形，不使用台湾、香港地区词汇
转换；该偏好统一应用于所有书籍，不在单本阅读器中单独配置。中文原著转换原文，
其他语言原著只转换中文译文；TXT、正式/临时译文和 EPUB 正文均只在最终显示层
处理。搜索会统一字形后匹配，朗读使用屏幕上的实际文本，转换资源由 PWA 缓存供
离线首次启用。书籍正文、译文、书签、进度和下载导出仍保存原始内容，不会写入
转换后的副本。

设置页中的语言检测阈值和自动翻译预翻译说明通过信息按钮查看：桌面端使用就近
气泡，手机端使用屏幕内弹窗，避免说明内容越出可视区域。百度翻译与有道翻译使用
各自官方开放 API，应用凭据统一保存在当前浏览器的全局设置中；交互翻译和小说
工具箱术语翻译直接读取这份配置，不再使用服务商网页接口。
...
阅读器会在隔离边界内保留 XHTML 结构、图片、SVG/MathML、表格、列表、本地
CSS/字体、音视频、横竖排版及内部/外部链接。完整兼容范围、降级行为与安全边界
见 [EPUB 支持范围](docs/epub-support.md)。

TXT 导入会先在专用 Worker 中检测 UTF-8、UTF-16、GB18030、Big5 与 Shift_JIS
编码，再按中日英标题规则和全文结构生成分层目录。每本 TXT 都必须先经过预览；
可选择严格、平衡或宽松模式，并可在导入前增删目录位置、修改标题与一至六级层级。
导入后可单独编辑目录显示文本；改变位置或层级必须执行完整重建。重建以浏览器中
当前保存的正文为准，按来源保留可证明完整的译文，并重定位进度与书签。识别规则、
误判边界、大文件行为和重建约束见 [TXT 目录支持范围](docs/txt-catalog-support.md)。
...
TXT/SRT 正文与翻译服务输出始终按纯文本处理；EPUB 只渲染经过清理的原书
结构，翻译输出不会被解释为 HTML。词典、朗读和“发送到交互翻译”均须由用户
主动触发；后者只在本次浏览器会话中交接选中文本，实际翻译仍使用用户已配置
的服务。

书籍有效语言包含中文时，阅读器直接显示原文，并隐藏整章未翻译状态、翻译
本页入口和阅读版本询问。文本选择、AI 查词和选中翻译仍然保留，可用于正文
中偶尔出现的外语句子。
...
- 底部“目录”按 EPUB 原生目录或已审阅的 TXT 目录展示树形层级，并标记当前章
  和翻译状态；“工具”集中提供全文搜索、书签列表、朗读、书籍信息和前后章入口。
  “AI”在阅读器内展开查词面板，选中文本后打开会自动填入输入框。
- 阅读流默认为“自动”：电脑使用双页横向分页，手机使用单栏纵向滚动。设置中
  也可固定为分页或滚动；分页支持左右边缘点击、滚轮、方向键、PageUp、
  PageDown 和空格键。
- 左右方向键可跳转上/下一章；数字键 1～4 可快速切换翻译
  （中文/中日/日中/原文）。切换显示模式时，分页以单列当前第一行或双列左列
  第一行为锚点，滚动模式以第一行完整可见文本为锚点。
