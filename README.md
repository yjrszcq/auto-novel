...
阅读设置可按标准 OpenCC 规则显示简体或繁体字形，不使用台湾、香港地区词汇
转换。中文原著转换原文，其他语言原著只转换中文译文；TXT、正式/临时译文和
EPUB 正文均只在最终显示层处理。搜索会统一字形后匹配，朗读使用屏幕上的实际
文本，转换资源由 PWA 缓存供离线首次启用。书籍正文、译文、书签、进度和下载
导出仍保存原始内容，不会写入转换后的副本。
