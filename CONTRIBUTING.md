...
请求并发和数量、共享 GPT 池的高水位/热加入/停止重排、独立接口限流、取消/恢复
落库范围、缓存身份/去重/容量、千章 IndexedDB 请求数以及长章节 DOM 窗口。修改
翻译算法、调度、缓存或阅读器数据层时应至少运行该命令。

GPT 工作区只有一个任务协调器，已启动的 GPT 卡片只向共享池提供请求容量；不要
重新引入“每张卡领取整项任务”的执行路径。每个工作者上限为 1–16，全池实际上限
是已启动工作者之和并受未完成工作高水位约束。Sakura 依赖译文前文，仍由独立的
顺序分段路径执行，不得接入 GPT 共享池。
